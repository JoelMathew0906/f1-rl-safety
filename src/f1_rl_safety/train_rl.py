...
def train_sb3_algo(algo: str, regime: RaceRegime, total_timesteps: int, seed: int,
                   log_dir: Path, model_dir: Path):
    discrete = algo == "dqn"
...
def main():
    parser = argparse.ArgumentParser(description="Unified training entrypoint for RL architectures.")
    parser.add_argument("--algo", type=str, choices=ALGOS, required=True,
                        help="RL algorithm: ppo, a2c, dqn, sarsa, reinforce")
    parser.add_argument("--regime", type=str, choices=list(REGIMES.keys()), required=True,
